...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    const placedTop = await dictionaryModel({ dict_coding: 'integrated_news_top' });
    updateSchema([
...
async function handleSubmit() {
    try {
        ...
        createMessage.success(!unref(isUpdate) ? t('router.common.addSuccess') : t('router.common.editSuccess'));
        closeModal();
        emit('success');
    } finally {
        setModalProps({ confirmLoading: false });
