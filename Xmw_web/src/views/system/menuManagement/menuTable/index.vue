    ...
    <div>
        <BasicTable @register="registerTable" @fetch-success="onFetchSuccess">
            ...
            <template #menu_type="{ record }">
                <Tag
                    :color="tagOptions[record.menu_type]"
                >{{ formatDictValue(options['menu_type'], record.menu_type) }}</Tag>
            </template>
            <!-- 是否隐藏-插槽 -->
            <template #hide_menu="{ record }">
                <Tag
                    :color="record.hide_menu === '0' ? 'success' : 'error'"
                    v-if="record.hide_menu"
                >{{ formatDictValue(options['hide_menu'], record.hide_menu) }}</Tag>
            </template>
            <!-- 是否忽略缓存-插槽 -->
            ...
            <template #affix="{ record }">
                <Tag
                    :color="record.affix === '0' ? 'success' : 'error'"
                >{{ formatDictValue(options['affix'], record.affix) }}</Tag>
            </template>
            <!-- 状态-插槽 -->
...
async function initOptions() {
    options['menu_type'] = await dictionaryModel({ dict_coding: 'system_menu_type' });
    options['hide_menu'] = await dictionaryModel({ dict_coding: 'system_isHide' });
    options['ignore_keep_alive'] = await dictionaryModel({ dict_coding: 'system_menu_keepAlive', });
    options['affix'] = await dictionaryModel({ dict_coding: 'system_menu_affix' });
    options['status'] = await dictionaryModel({ dict_coding: 'system_status' });
    options['hide_childrenIn_menu'] = await dictionaryModel({ dict_coding: 'system_menu_hideChild', });
}
initOptions();
//新增操作
